...
description_data_html <- html_nodes(webpage,'#main p:nth-child(3) , p.text-muted:nth-child(4)')
...
#Data-Preprocessing: converting it to numerical
runtime_data<-as.numeric(runtime_data)

#Let's have another look at the runtime data
head(runtime_data)

#Using CSS selectors to scrap the Movie rating section
viewer_rating_data_html <- html_nodes(webpage,'.certificate')

#Convert to text
viewer_rating_data <- html_text(viewer_rating_data_html)

length(viewer_rating_data)

#Change missing values to NA
for (i in c(4,22,40,41,46)){
  
  a<-viewer_rating_data[1:(i-1)]
  
  b<-viewer_rating_data[i:length(viewer_rating_data)]
  
  viewer_rating_data<-append(a,list("NA"))
  
  viewer_rating_data<-append(viewer_rating_data,b)
...
a<-directors_data[1:(13)]
b<-directors_data[15:length(directors_data)]
directors_data<-append(a,b)
...
movies_df<-data.frame(Rank = rank_data, Title = title_data,
                      ...
                      Genre = genre_data, Rating = rating_data, Metascore_Rating=metascore_data,
                      Gross_Earning_in_Mil = gross_data, Votes_Count=votes_data,
...
write.csv(movies_df,"50popularmovies2019.csv")
